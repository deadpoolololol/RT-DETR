...
class BaseSolver(object):
    ...
    def setup(self, ):
        ...
        if self.cfg.resume: # 如果恢复则在相同目录下
            time_pattern = re.search(r"(\d{8}_\d{6})", self.cfg.resume)
            self.time = time_pattern.group(1)

        self.output_dir = self.output_dir / self.time # 添加训练开始时间
        
        self.visualizer = Visualizer(self.output_dir) # 初始化可视化
